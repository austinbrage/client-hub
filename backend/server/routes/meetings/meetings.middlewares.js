...
import { MeetingsStringValidation } from './meetings.validations.js';

class SanitizeGetRequests {

    constructor() {
        this.validateStringMeetings = new MeetingsStringValidation();
    }
    ...
    getAllByClient = (req, res, next) => {
        const validation = this.validateStringMeetings.getAllByClient(req.query);
    ...
    getId = (req, res, next) => {
        const validation = this.validateStringMeetings.getId(req.query);
        if (!validation.success) return this.validationErr(res, validation.error);
